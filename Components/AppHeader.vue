<template>
  <v-app-bar color="sandy" class="position-relative" scroll-behavior="hide elevate" floating dark v-if="headerData"
    flat>
    <v-col class="d-flex ga-3">
      <v-toolbar-title @click="$router.push('/')" v-if="headerData.name"
        class="header__name flex-0-0 d-flex align-center justify-center pl-2 p6 cursor-pointer">
        {{ headerData.name }}
      </v-toolbar-title>
      <v-btn v-if="headerData.btnLeft" class="text-none rounded-pill" color="btnBg" min-height="48" variant="flat">
    ...
    <v-btn v-if="headerData.btnLeft" class="text-none rounded-pill pl-3 pr-1" color="btnBg" min-height="48"
      variant="flat">
